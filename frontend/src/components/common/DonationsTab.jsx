...
function ReceiptCell({ charityId, charityName, receipts, userNames, onChanged }) {
  ...
  const processFile = useCallback((file) => {
    ...
    reader.onload = async () => {
      setUploading(true);
      try {
        await donationsApi.addReceipt(charityId, { fileName: file.name, data: reader.result, donatedBy: donatedBy.trim() });
        onChanged?.();
        setAdding(false);
        setDonatedBy('');
      } catch {
  ...
  const remove = useCallback(async (receiptId) => {
    try {
      await donationsApi.removeReceipt(charityId, receiptId);
      onChanged?.();
    } catch {
      // silent fail
    }
  ...
  return (
    <div style={{ display: 'grid', gap: '0.3rem' }}>
      {receipts.map((r) => (
        <div key={r.id} style={{ display: 'grid', gap: '0.1rem', fontSize: '0.78rem', borderBottom: '1px solid #fde68a', paddingBottom: '0.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            ...
            <button onClick={() => remove(r.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: '0.85rem', padding: '0 0.2rem', lineHeight: 1 }} title="Remove">✕</button>
          </div>
          {r.donatedBy && (
        ...
        <div style={{ display: 'grid', gap: '0.35rem', background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '0.5rem', padding: '0.4rem', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: '0.3rem' }}>
            <button
              ...
              disabled={uploading}
              style={{ flex: 1, background: '#15803d', border: 'none', borderRadius: '0.35rem', color: '#fff', fontSize: '0.75rem', padding: '0.28rem 0.4rem', cursor: uploading ? 'default' : 'pointer', fontWeight: 600, opacity: uploading ? 0.6 : 1 }}
            >
              {uploading ? '⏳' : '📁 File'}
            </button>
            <button
              ...
              disabled={uploading}
              style={{ flex: 1, background: '#0369a1', border: 'none', borderRadius: '0.35rem', color: '#fff', fontSize: '0.75rem', padding: '0.28rem 0.4rem', cursor: uploading ? 'default' : 'pointer', fontWeight: 600, opacity: uploading ? 0.6 : 1 }}
            >
              📷 Camera
            </button>
...
export default function DonationsTab() {
  ...
  const refreshReceipts = useCallback(async () => {
    try { setReceipts(await donationsApi.getReceipts()); } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    donationsApi.getReceipts().then(setReceipts).catch(() => {});
  ...
  // Count charities that have at least one server receipt
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
            ...
            <tbody>
              {charities.map((charity) => (
                <tr key={charity.charityId} style={{ borderBottom: '1px solid #fde68a', verticalAlign: 'top' }}>
                  ...
                  <td style={{ ...td, minWidth: '140px' }}>
                    <ReceiptCell charityId={charity.charityId} charityName={charity.name} receipts={receipts[charity.charityId] || []} userNames={userNames} onChanged={refreshReceipts} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <p style={{ fontSize: '0.78rem', color: '#9ca3af', margin: 0 }}>
        Employer match is 100% of the total donated to each charity. Click "🤝 Match" to register your pledge.
      </p>
    </div>
  );
}
